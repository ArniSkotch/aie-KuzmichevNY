...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # НОВЫЕ ЭВРИСТИКИ:
    # 1. Проверка на константные столбцы
    constant_columns = []
    for col in summary.columns:
        if col.unique == 1 and col.non_null > 0:  # все значения одинаковы и не все пропущены
    ...
    flags["constant_columns_names"] = constant_columns

    # 2. Проверка на высокую кардинальность категориальных признаков
    high_cardinality_columns = []
    for col in summary.columns:
        # проверяем нечисловые столбцы с большим числом уникальных значений
        if not col.is_numeric and col.unique > summary.n_rows * 0.8:  # более 80% уникальных значений
            high_cardinality_columns.append(col.name)
    flags["has_high_cardinality_categoricals"] = len(high_cardinality_columns) > 0
    flags["high_cardinality_columns_count"] = len(high_cardinality_columns)
    flags["high_cardinality_columns_names"] = high_cardinality_columns

    # Обновляем quality_score с учетом новых эвристик
    score = 1.0
    ...
    if flags["has_constant_columns"]:
        score -= 0.1 * min(flags["constant_columns_count"] / summary.n_cols, 1.0)
    if flags["has_high_cardinality_categoricals"]:
        score -= 0.05 * min(flags["high_cardinality_columns_count"] / summary.n_cols, 1.0)
