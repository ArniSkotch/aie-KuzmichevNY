...
def report(
    ...
    min_missing_share: float = typer.Option(0.3, help="Порог доли пропусков для определения проблемных столбцов."),
) -> None:
    """
    ...
    top_cats = top_categories(df, max_columns=10, top_k=top_k_categories)  # используем новый параметр
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        if quality_flags['has_constant_columns']:
            f.write(f"  - Количество константных столбцов: {quality_flags['constant_columns_count']}\n")
            f.write(f"  - Имена константных столбцов: {', '.join(quality_flags['constant_columns_names'])}\n")
        ...
        if quality_flags['has_high_cardinality_categoricals']:
            f.write(f"  - Количество таких столбцов: {quality_flags['high_cardinality_columns_count']}\n")
            f.write(f"  - Имена столбцов: {', '.join(quality_flags['high_cardinality_columns_names'])}\n")
        f.write("\n")
        ...
        # информация о проблемных столбцах по пропускам
        f.write("## Проблемные столбцы (по пропускам)\n\n")
        problem_columns = missing_df[missing_df["missing_share"] > min_missing_share]
        if not problem_columns.empty:
            f.write("Следующие столбцы имеют долю пропусков выше указанного порога:\n\n")
            f.write(problem_columns.to_markdown())
        else:
            f.write("Нет столбцов с долей пропусков выше указанного порога.\n\n")
