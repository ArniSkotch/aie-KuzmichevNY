...
@app.post(
    ...
    tags=["quality"],
    summary="Возвращает полный набор флагов качества для CSV-файла",
    response_description="Полный словарь флагов качества, включая новые эвристики из HW03",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict:
    ...
    Этот эндпоинт возвращает все флаги, включая новые эвристики из hw03:
    ...
    - и другие стандартные флаги качества
    """
    start = perf_counter()

    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
        # content_type от браузера может быть разным, поэтому проверка мягкая
        # но оставим простую ветку 400 для вида
        raise HTTPException(status_code=400, detail="Ожидается CSV-файл (content-type text/csv).")
    ...
    # Добавляем метаинформацию в ответ
    flags_all["latency_ms"] = latency_ms
    flags_all["filename"] = file.filename
    flags_all["n_rows"] = summary.n_rows
    flags_all["n_cols"] = summary.n_cols
